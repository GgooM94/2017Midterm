...
public class MonsterCtrl : MonoBehaviour {
	//몬스터 상태 정보가 있는 Enumerable 변수 선언
	...
	void Awake () {
		...
		//GameUI 게임 오브젝트의 GameUI 스크립트를 할당
		gameUI = GameObject.Find("GameUI").GetComponent<GameUI>();

    //     monsterFirePos = GetComponent<Transform>();

		/*추적 대상의 위치를 설정하면 추적 시작
	...
	//OnEnable , OnDisable 스크립트가 활성화되거나 비활성화 될때 수행되는 함수
	//(이벤트가 선언된 클래스명).(이벤트명) += (이벤트 발생 시 호출할 함수)
	//이벤트 발생 시 수행할 함수 연결
	void OnEnable(){
		PlayerCtrl.OnPlayerDie += this.OnPlayerDie;

		//일정한 간격으로 몬스터의 행동 상태를 체크하는 코루틴 함수실행
	...
	//일정한 간격으로 몬스터의 행동 상태를 체크하고 mosterState 값 변경
			...
			switch (monsterState)
			{
				...
				case MonsterState.attack:
                   ...
                   /* 
                    ...
                    GameObject fire = (GameObject)Instantiate(monsterFire, monsterFirePos.position, Quaternion.identity);
                    //ParticleSystem 컴포넌트의 수행시간(duration)이 지난 후  삭제 거리
                    Destroy(fire, fire.GetComponent<ParticleSystem>().duration + 0.2f);
	...
	void CreateBloodEffect(Vector3 pos){
		...
		//Decal Create Location - 바닥에서 조금 올린 위치 산출
		Vector3 decalPos = monsterTr.position + (Vector3.up *0.05f);

		//Decal RotationData Random Create
